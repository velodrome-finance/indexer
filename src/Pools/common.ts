...
export type PoolLoaderError<T extends PoolLoaderErrorType> =
  T extends "TokenNotFoundError"
    ? PoolLoaderTokenError
    : PoolLoaderLiquidityPoolError;
...
export async function fetchPoolLoaderData(
  ...
  const liquidityPoolAggregator =
    await context.LiquidityPoolAggregator.get(liquidityPoolAddress);
