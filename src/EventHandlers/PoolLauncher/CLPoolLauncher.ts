...
CLPoolLauncher.PairableTokenAdded.handler(async ({ event, context }) => {
  ...
  // Get or create CLPoolLauncherConfig
  let config = await context.CLPoolLauncherConfig.get(configId);
  ...
  context.CLPoolLauncherConfig.set(config);
...
CLPoolLauncher.PairableTokenRemoved.handler(async ({ event, context }) => {
  ...
  // Get existing CLPoolLauncherConfig
  const config = await context.CLPoolLauncherConfig.get(configId);
  ...
  if (config) {
    ...
    context.CLPoolLauncherConfig.set(updatedConfig);
    context.log.info(`Pairable token removed: ${tokenAddress}`);
  } else {
    context.log.warn(
      `CLPoolLauncherConfig not found for token removal: ${configId}`,
    );
  }
});
...
CLPoolLauncher.NewPoolLauncherSet.handler(async ({ event, context }) => {
  ...
  const existingConfig = await context.CLPoolLauncherConfig.get(oldConfigId);
  ...
  if (existingConfig) {
    ...
    // Set the new config
    context.CLPoolLauncherConfig.set(newConfig);
    ...
    context.log.warn(
      `CLPoolLauncherConfig not found for pool launcher update: ${oldConfigId}`,
    );
  }
});
