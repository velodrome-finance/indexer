...
describe("CLPoolLauncher Events", () => {
  ...
  describe("CLPoolLauncher.PairableTokenAdded", () => {
    it("should create new CLPoolLauncherConfig when adding first token", async () => {
      ...
      // Should create new CLPoolLauncherConfig
      const config = result.entities.CLPoolLauncherConfig.get(configId);
      expect(config).to.not.be.undefined;
      expect(config?.id).to.equal(configId);
      expect(config?.pairableTokens).to.deep.equal([tokenAddress]);
    });

    it("should add token to existing CLPoolLauncherConfig", async () => {
      ...
      mockDb = mockDb.entities.CLPoolLauncherConfig.set(existingConfig);
      ...
      // Should update existing CLPoolLauncherConfig
      const config = result.entities.CLPoolLauncherConfig.get(configId);
      expect(config).to.not.be.undefined;
      expect(config?.id).to.equal(configId);
    ...
    it("should not add duplicate token to CLPoolLauncherConfig", async () => {
      const tokenAddress = "0x8888888888888888888888888888888888888888";
      const configId = `${mockChainId}-${mockLauncherAddress}`;
  ...
  describe("CLPoolLauncher.PairableTokenRemoved", () => {
    it("should remove token from existing CLPoolLauncherConfig", async () => {
      ...
      // Should update CLPoolLauncherConfig by removing the token
      const config = result.entities.CLPoolLauncherConfig.get(configId);
      expect(config).to.not.be.undefined;
      expect(config?.id).to.equal(configId);
  ...
  describe("CLPoolLauncher.NewPoolLauncherSet", () => {
    it("should update CLPoolLauncherConfig ID when pool launcher changes", async () => {
      ...
      const newConfig = result.entities.CLPoolLauncherConfig.get(newConfigId);
      ...
      const oldConfig = result.entities.CLPoolLauncherConfig.get(oldConfigId);
