...
describe("CLGauge Event Handlers", () => {
  ...
  describe("Event Data Mapping", () => {
    it("should map CLGauge.Deposit event data correctly", async () => {
      ...
      expect(mockEvent.params.user).to.equal(mockUserAddress);
      expect(mockEvent.params.liquidityToStake).to.equal(
        100000000000000000000n,
      );
      expect(mockEvent.params.tokenId).to.equal(1n);
    ...
    it("should map CLGauge.Withdraw event data correctly", async () => {
      ...
      // Test that the event data is correctly structured
      expect(mockEvent.params.user).to.equal(mockUserAddress);
      expect(mockEvent.params.liquidityToStake).to.equal(50000000000000000000n);
      expect(mockEvent.params.tokenId).to.equal(1n);
      expect(mockEvent.srcAddress).to.equal(mockGaugeAddress);
      expect(mockEvent.chainId).to.equal(mockChainId);
